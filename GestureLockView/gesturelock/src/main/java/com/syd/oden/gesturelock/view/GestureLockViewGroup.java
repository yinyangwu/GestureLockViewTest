...
import com.youngwu.gesturelock.R;
...
public class GestureLockViewGroup extends RelativeLayout {
    ...
    public GestureLockViewGroup(Context context, AttributeSet attrs, int defStyle) {
        ...
        int mPreferenceId = a.getInt(R.styleable.GestureLockViewGroup_preference_id, -1);
    ...
    private void initViews() {
        ...
        if (mGestureLockViews == null) {
            ...
            int mMarginBetweenHorizontalLockView;
            int mMarginBetweenVerticalLockView;
            if (mWidth <= mHeight) {
                // 竖屏模式下计算每个GestureLockView的宽度
                mGestureLockViewWidth = (int) (4 * mWidth * 1.0f / (5 * mCount + 1));
                //计算每个横向GestureLockView的间距
                mMarginBetweenHorizontalLockView = (int) (mGestureLockViewWidth * 0.25);
                //计算每个纵向GestureLockView的间距
                mMarginBetweenVerticalLockView = (mHeight - mCount * mGestureLockViewWidth) / 4;
            } else {
                // 横屏模式下计算每个GestureLockView的宽度
                mGestureLockViewWidth = (int) (4 * mHeight * 1.0f / (5 * mCount + 1));
                //计算每个纵向GestureLockView的间距
                mMarginBetweenVerticalLockView = (int) (mGestureLockViewWidth * 0.25);
                //计算每个横向GestureLockView的间距
                mMarginBetweenHorizontalLockView = (mWidth - mCount * mGestureLockViewWidth) / 4;
            }
            ...
            for (int i = 0; i < mGestureLockViews.length; i++) {
                ...
                if (i < mCount) {
                    // 第一行
                    topMargin = mMarginBetweenVerticalLockView;
                }
                if (i % mCount == 0) {
                    // 第一列
                    leftMargin = mMarginBetweenHorizontalLockView;
                }

                lockerParams.setMargins(leftMargin, topMargin, mMarginBetweenHorizontalLockView, mMarginBetweenVerticalLockView);
            ...
            case MotionEvent.ACTION_UP:
                if (isInPasswordSettingMode) {
                    ...
                    if (gestureEventListener != null) {
                        gestureEventListener.onGestureEvent(isCorrect, mTryTimes);//将结果回调
                    }
                    if (this.mTryTimes == 0) {
    ...
    private void setItemModeUp() {
        for (GestureLockView verticalGestureLockView : mGestureLockViews) {
            if (mChoose.contains(verticalGestureLockView.getId())) {
                verticalGestureLockView.setMode(Mode.STATUS_FINGER_UP);
            }
        }
    }
    ...
    private void reset() {
        ...
        mPath.reset();
        for (GestureLockView verticalGestureLockView : mGestureLockViews) {
            verticalGestureLockView.setMode(Mode.STATUS_NO_FINGER);
